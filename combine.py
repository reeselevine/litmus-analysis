...
def max_sum(all_stats):
    analyze_combined(all_stats, "sum", maximize_sum(calculate_total))
...
def max_log_rate(all_stats):
    def calculate(stats, key, testKey):
        return math.log(calculate_rate(stats, key, testKey) + 1, 2)
    analyze_combined(all_stats, "log rate", maximize_sum(calculate), compare_greater_than)
...
def max_ceiling_log_rate(all_stats):
    ...
    analyze_combined(all_stats, "log rate", max_log_ceiling, compare, (0, 0))
...
def main():
    ...
    parser.add_argument("--action", default="sum", help="Analysis to perform. Options are 'sum', 'rate', 'log-sum', 'log-rate', 'tanh-rate', 'min-rate', 'ceiling-log-rate'")
